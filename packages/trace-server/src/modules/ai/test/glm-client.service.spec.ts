...
describe('GlmClientService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [GlmClientService, { provide: ConfigService, useValue: { get: mockConfigGet } }],
    }).compile();
  ...
  describe('正常调用', () => {
    it('成功返回 AI 文本和 token 用量', async () => {
      const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(mockFetchSuccess('这是 AI 的分析结果') as any);
    ...
    it('默认 temperature=0.3, maxTokens=1024', async () => {
      const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(mockFetchSuccess('ok') as any);
  ...
  describe('错误处理与重试', () => {
    it('401 错误：应直接抛异常，不重试', async () => {
      const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(mockFetchError(401, 'Invalid API Key') as any);
    ...
    it('429 限流：应直接抛异常，不重试', async () => {
      const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(mockFetchError(429, 'Rate limit exceeded') as any);
    ...
    it('500 错误：应重试 1 次，两次都失败后抛出异常', async () => {
      const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue(mockFetchError(500, 'Server Error') as any);
    ...
    it('网络错误（fetch 本身抛异常，无 HTTP 状态码）：应重试', async () => {
      ...
      const fetchSpy = jest.spyOn(global, 'fetch').mockRejectedValueOnce(networkError).mockRejectedValueOnce(networkError);
